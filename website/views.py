...
@login_required
def profile(request):
    table_bookings = Booking.objects.filter(user=request.user).exclude(status='cancelled')
    room_bookings = RoomBooking.objects.filter(user=request.user).exclude(status='cancelled')
